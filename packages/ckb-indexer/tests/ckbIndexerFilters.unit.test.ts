...
test('should isCellAfterCursor work fine', async (t) => {
  ...
  t.false(isCellAfterCursor({ cell: mockCell, cursor: cursorOfCell(mockCell) }));
...
test('should filterByIndexerFilterProtocol function filters cells with indexer filter options', async (t) => {
  ...
  t.deepEqual(filterByIndexerFilterProtocol({ cells, params: indexerFilter }), cells);
  ...
  t.deepEqual(
    filterByIndexerFilterProtocol({
      cells,
      params: { searchKey: { script: dummyScript, scriptType: 'lock', filter: { scriptLenRange: ['0x0', '0x1'] } } },
    }),
    [cells[1]],
  );
  t.deepEqual(
    filterByIndexerFilterProtocol({
      cells,
      params: { searchKey: { script: dummyScript, scriptType: 'type', filter: { scriptLenRange: ['0x0', '0x1'] } } },
    }),
    [],
  );
  ...
  t.deepEqual(
    filterByIndexerFilterProtocol({
      cells,
      params: { searchKey: { script: dummyScript, scriptType: 'lock', filter: { script: { ...dummyScript, codeHash: '0x00' } } } },
    }),
    [cells[0]],
  );
