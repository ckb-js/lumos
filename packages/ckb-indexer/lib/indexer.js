...
exports.CkbIndexer = exports.Order = exports.ScriptType = void 0;
...
const DefaultTerminator = (_index, _cell) => {
    return { stop: false, push: true };
};
class CkbIndexer {
    constructor(ckbRpcUrl, ckbIndexerUrl) {
        this.ckbRpcUrl = ckbRpcUrl;
        this.ckbIndexerUrl = ckbIndexerUrl;
        this.uri = ckbRpcUrl;
    }
    getCkbRpc() {
        return new rpc_1.RPC(this.ckbRpcUrl);
    }
    async tip() {
    ...
    collector(queries) {
        ...
        if (lock !== undefined) {
            ...
            if (type != undefined && type != "empty") {
                ...
                throw new Error(`should specify either type or lock in queries, queries now: ${JSON.stringify(queries, null, 2)}`);
            }
        }
    ...
    async request(method, params, ckbIndexerUrl = this.ckbIndexerUrl) {
        const data = {
            id: 0,
            jsonrpc: "2.0",
            method,
        ...
        const res = await axios_1.default.post(ckbIndexerUrl, data);
        if (res.status !== 200) {
