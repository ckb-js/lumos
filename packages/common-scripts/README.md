...
Below are two examples showing how to use `p2pkh` script.
```ts
// Omni lock example
const inputCells: Cell[] = {...};
const outputCells: Cell[] = {...};
tx = tx.update("inputs", (inputs) => inputs.push(...inputCells));
tx = tx.update("outputs", (outputs) => outputs.push(...outputCells));
...
const messageGroup = createP2PKHMessageGroup(tx, validLocks, {
  update: (message) => hasher.update(message),
  digest: () => hasher.digestHex(),
})

// for each Group, sign the message with the lock's corresponding private key, and update witness[index] with the signature.
...
const messageGroup = createP2PKHMessageGroup(tx, [validLock], {
  update: (message) => {
