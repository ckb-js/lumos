...
import { locktimePool, LocktimeCell, FromInfo, secp256k1Blake160Multisig } from "../src";
const { prepareSigningEntries, payFee } = locktimePool;
import { CellProvider } from "./cell_provider";
import { List } from "immutable";
import { DEV_CONFIG } from "./dev_config";
import { Config, predefined } from "@ckb-lumos/config-manager";
import { Header, Cell, CellCollector, Script, since as SinceUtils } from "@ckb-lumos/base";
...
test("JSBI:payFee, multisig & dao", async (t) => {
  ...
  txSkeleton = await payFee(txSkeleton, [fromInfo, aliceAddress], BI.from(fee), tipHeader, {
    config: DEV_CONFIG,
...
test("CellCollector, multisig", async (t) => {
  ...
  const multisigScript = secp256k1Blake160Multisig.serializeMultisigScript(bob.fromInfo);
  ...
  const collector = new locktimePool.CellCollector(bob.multisigTestnetAddress, cellProvider, {
    config: AGGRON4,
    NodeRPC: RpcMocker as any,
    tipHeader,
  });
...
test("CellCollector, dao & multisig, dao since > multisig since", async (t) => {
  ...
  const multisigScript = secp256k1Blake160Multisig.serializeMultisigScript(bob.fromInfo);
  const args = secp256k1Blake160Multisig.multisigArgs(multisigScript, multisigSince);
  ...
  t.is(BI.from(cell.cellOutput.capacity).toString(), maximumWithdrawCapacity.toString());
});
