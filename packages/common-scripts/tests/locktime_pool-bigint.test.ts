...
import { bobSecpDaoDepositInput, bobSecpDaoWithdrawInput, tipHeader as inputTipHeader } from "./inputs";
...
test("BigInt:transfer multisig", async (t) => {
  txSkeleton = await transfer(txSkeleton, [fromInfo, aliceAddress], bobAddress, BigInt(500 * 10 ** 8), tipHeader, {
    config: DEV_CONFIG,
...
test("BigInt:transfer multisig & dao", async (t) => {
  txSkeleton = await transfer(txSkeleton, [fromInfo, aliceAddress], bobAddress, BigInt(2500 * 10 ** 8), tipHeader, {
    config: DEV_CONFIG,
    LocktimeCellCollector,
  });
...
test("BigInt:prepareSigningEntries, multisig & dao", async (t) => {
  ...
  expectedMessages.forEach((expectedMessage, index) => {
    const message = txSkeleton.get("signingEntries").find((s) => s.type === "witness_args_lock" && s.index === index)!
      .message;
...
test("BigInt:payFee, multisig & dao", async (t) => {
  ...
  t.is(interest, sumOfInputCapacity - BigInt(originCapacity) * BigInt(txSkeleton.get("inputs").size));
...
test("BigInt:Don't update capacity directly when deduct", async (t) => {
  ...
  txSkeleton = await transfer(txSkeleton, [fromInfo, aliceAddress], bobAddress, BigInt(600 * 10 ** 8), tipHeader, {
    config: DEV_CONFIG,
    LocktimeCellCollector,
  });
  ...
  try {
    await transfer(txSkeleton, [fromInfo, aliceAddress], bobAddress, BigInt(500 * 10 ** 8), tipHeader, {
      config: DEV_CONFIG,
      LocktimeCellCollector,
    });
  } catch {
...
test("BigInt:CellCollector, dao", async (t) => {
  ...
  t.is(BigInt(cell.cellOutput.capacity), maximumWithdrawCapacity);
});
