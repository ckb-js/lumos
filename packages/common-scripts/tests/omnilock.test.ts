...
test("JSBI:transfer success", async (t) => {
  txSkeleton = await omnilock.transferCompatible(
    txSkeleton,
    bob.mainnetAddress,
...
test("JSBI:transfer to non omnilock address", async (t) => {
  txSkeleton = await omnilock.transferCompatible(
    txSkeleton,
    bob.mainnetAddress,
  ...
  const template = LINA.SCRIPTS.OMNILOCK!;
...
test("JSBI:payFee", async (t) => {
  ...
  txSkeleton = await omnilock.payFee(
    txSkeleton,
    bob.mainnetAddress,
    BI.from(fee)
  );
...
test("JSBI:prepareSigningEntries", async (t) => {
  ...
  txSkeleton = await omnilock.prepareSigningEntries(txSkeleton);
...
test("setupInputCell", async (t) => {
  ...
  txSkeleton = await omnilock.setupInputCell(
    txSkeleton,
    inputCell,
    undefined,
    {
...
test("injectCapacity", async (t) => {
  ...
  txSkeleton = await omnilock.injectCapacity(
    txSkeleton,
    0,
    bob.testnetAddress,
    {
