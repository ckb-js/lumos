...
const multisigInput: Cell = {
  ...
  outPoint: {
    txHash: "0xc0018c999d6e7d1f830ea645d980a3a9c3c3832d12e72172708ce8461fc5821e",
    index: "0x1",
  },
  blockHash: "0x29c8f7d773ccd74724f95f562d049182c2461dd7459ebfc494b7bb0857e8c902",
  blockNumber: "0x1aed9",
  data: "0x",
};
...
test("BigInt:_commonTransfer, only alice", async (t) => {
  ...
  const result = await _commonTransfer(txSkeleton, [aliceAddress], amount, BigInt(61 * 10 ** 8), { config: AGGRON4 });
...
test("BigInt:_commonTransfer, alice and fromInfo", async (t) => {
  const amount: bigint = BigInt(20000 * 10 ** 8);
  const result = await _commonTransfer(txSkeleton, [aliceAddress, fromInfo], amount, BigInt(61 * 10 ** 8), {
    config: AGGRON4,
  });
  txSkeleton = result.txSkeleton;
