...
import { parseAddress, TransactionSkeletonType, Options, minimalCellCapacityCompatible } from "@ckb-lumos/helpers";
...
export async function transfer(
  ...
  const fromScripts: Script[] = fromInfos.map((fromInfo) => parseFromInfo(fromInfo, { config }).fromScript);
  ...
  for (const { index, cellCollector, isAnyoneCanPay, destroyable } of cellCollectorInfos) {
    for await (const inputCell of cellCollector.collect()) {
      ...
      let deductCapacity: BI =
        isAnyoneCanPay && !destroyable ? inputCapacity.sub(minimalCellCapacityCompatible(inputCell)) : inputCapacity;
      ...
      if (
        ...
        ((changeCapacity.eq(0) && changeAmount.eq(0)) ||
          (changeCapacity.gt(minimalCellCapacityCompatible(changeCellWithoutSudt)) && changeAmount.eq(0)))
      ) {
  ...
  if (
    ...
    if (changeAmount.gt(0)) {
      clonedOutput.data = toBigUInt128LE(readBigUInt128LECompatible(originOutput.data).add(changeAmount));
    }

    const minimalChangeCellCapcaity = BI.from(minimalCellCapacityCompatible(changeCell));
    const minimalChangeCellWithoutSudtCapacity = BI.from(minimalCellCapacityCompatible(changeCellWithoutSudt));
    let splitFlag = false;
    if (
      changeAmount.gt(0) &&
      splitChangeCell &&
