...
import { Cell, QueryOptions, TransactionWithStatus, indexer as BaseIndexerModule } from "@ckb-lumos/base";
...
const mockTxs: TransactionWithStatus[] = [
  {
    transaction: {
      ...
      inputs: [
        {
          previousOutput: {
            txHash: "0x58a29007f29ede069d49221f468107681c1a4d8d341de1d053b9b60596d6b233",
            index: "0x0",
          },
          since: "0x0",
        },
    ...
    txStatus: {
      status: "committed",
      blockHash: "0x8df4763d10cf22509845f8ec728d56d1027d4dfe633cb91abf0d751ed5d45d68",
    },
  },
  {
    transaction: {
      ...
      inputs: [
        {
          previousOutput: {
            txHash: "0xfd69760e8062dca9142a6802d7f42f82204e1b266719e34a17cc1f5c0bd03b97",
            index: "0x0",
          },
          since: "0x0",
        },
...
tipStub.resolves({
  blockHash: "0xb97f00e2d023a9be5b38cc0dabcfdfa149597a3c5f6bc89b013c2cb69e186432",
  blockNumber: "0x10",
});
const cacheManager = CacheManager.fromMnemonic(indexer, mnemonic, getDefaultInfos(), {
...
test("getNextReceivingPublicKeyInfo", async (t) => {
  ...
  t.is(cacheManager.getNextReceivingPublicKeyInfo().blake160, "0xc337da539e4d0b89daad1370b945f7210fad4c43");
});
...
test("getNextChangePublicKeyInfo", async (t) => {
  ...
  t.is(cacheManager.getNextChangePublicKeyInfo().blake160, "0xbba6e863e838bae614fd6df9828f3bf1eed57964");
});
...
test("CellCollector", async (t) => {
  ...
  t.is(firstCell.blockHash, "0x8df4763d10cf22509845f8ec728d56d1027d4dfe633cb91abf0d751ed5d45d68");
});
...
test("CellCollectorWithQueryOptions", async (t) => {
  ...
  const cellCollector = new CellCollectorWithQueryOptions(new CellCollector(cacheManager), queryOptions);
...
test("CellCollectorWithQueryOptions, skip", async (t) => {
  ...
  t.deepEqual(
    cells.map((cell) => BI.from(cell.cellOutput.capacity).toString()),
    [BI.from(300).mul(BI.from(10).pow(8)).toString(), BI.from(400).mul(BI.from(10).pow(8)).toString()]
  );
});
...
test("getBalance", async (t) => {
  ...
  t.is(BI.from(balance).toString(), BI.from(900).mul(BI.from(10).pow(8)).toString());
});
...
test("getBalance, needMasterPublicKey", async (t) => {
  ...
  const balance = await getBalance(new CellCollector(cacheManager));
  t.is(BI.from(balance).toString(), BI.from(950).mul(BI.from(10).pow(8)).toString());
});
