...
test("validateSince, relative blockTimestamp", (t) => {
  ...
  const validTipMedianTimestamp = "0x" + BI.from(cellMedianTimestamp).add(timestamp.mul(1000)).toString(16);
  const invalidTipMedianTimestamp = "0x" + BI.from(cellMedianTimestamp).add(timestamp.mul(1000)).sub(1).toString(16);
  t.true(
    validateSince(
      since,
      {
