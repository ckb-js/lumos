...
test("serialize transaction", (t) => {
  ...
  const packUnpackSerializedHex = bytes.hexify(
    blockchain.Transaction.pack(blockchain.Transaction.unpack(serializedHex))
  );
  t.deepEqual(packUnpackSerializedHex, serializedHex);
});
