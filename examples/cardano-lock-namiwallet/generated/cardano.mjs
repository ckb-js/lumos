...
export function SerializeByte64(value) {
...
export class CardanoWitnessLock {
  ...
  validate(compatible = false) {
    ...
    new Byte64(this.view.buffer.slice(offsets[1], offsets[2]), { validate: false }).validate();
  ...
  getSignature() {
    ...
    const offset = this.view.getUint32(start, true);
    const offset_end = this.view.getUint32(start + 4, true);
    return new Byte64(this.view.buffer.slice(offset, offset_end), { validate: false });
  }
...
export function SerializeCardanoWitnessLock(value) {
  const buffers = [];
